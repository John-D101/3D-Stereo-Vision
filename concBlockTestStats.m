...
sampleLeftImages = imageDatastore(fullfile(sampleImageDir,"concrete block\left\"));
sampleRightImages = imageDatastore(fullfile(sampleImageDir,"concrete block\right\"));
...
for image_num = 1:size(sampleLeftImages.Files,1)
    ...
    % Using multi modal analysis so keeping main body of concrete only
    ptCloudRed = select(ptCloudNoise, findPointsInROI(ptCloudNoise, [-0.2 0.2 -0.5 0.5 -0.5 0.2]));
...
%% Fit a surface plane to each of the point clouds
...
for pc_num = 1:2:size(ptCloudGrouped,2)*2
    %Include all points that are within 1cm of the proposed plane
    [model,inlierIndices,outlierIndices,meanError] = pcfitplane(ptCloudGrouped{(pc_num+1)/2}, 0.01);
    % Do not trim the point cloud since we want to see if the cracks show
