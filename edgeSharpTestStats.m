...
sampleLeftImages = imageDatastore(fullfile(sampleImageDir,"edge sharpness test\left\"));
sampleRightImages = imageDatastore(fullfile(sampleImageDir,"edge sharpness test\right\"));
...
%% Fit a cylinder to each of the point clouds
%fittedCylModel stores the plane model inc. eq. on first row and
%histogram of distances on second row
fittedCylModel = cell(2,size(ptCloudGrouped,2)*2);
...
%Setting grid text
gridText = ["5cm", "5cm", "5cm", "5cm", ...
    "4cm", "4cm", "4cm", "4cm", ...
    "3cm", "3cm", "3cm", "3cm", ...
    "2cm", "2cm", "2cm", "2cm", ...
    "1.8cm", "1.8cm", "1.8cm", "1.8cm", "1.8cm", ...
    "1.6cm", "1.6cm", "1.6cm", "1.6cm", ...
...
for pc_num = 1:2:size(ptCloudTrimmed,2)*2
    ...
    normVec = fittedCylModel{1,pc_num}.Orientation;
    radius = fittedCylModel{1,pc_num}.Radius;
    ptVec = zeros(1,3);
    ...
    for pt_num = 1:size(ptsInPtCloud,1)
        ...
        if ~(isnan(ptsInPtCloud(pt_num,1)) || isnan(ptsInPtCloud(pt_num,2)) || isnan(ptsInPtCloud(pt_num,3)))
            ...
            distArray(1,pt_num) = norm(ptVec - dot(ptVec,normVec)/dot(normVec,normVec)*normVec)-radius;
        end
    end
    ...
    fittedCylModel(2,pc_num) = {histogram(distArray)};
    xlabel("Perpendicular distance from cylinder surface (m)")
